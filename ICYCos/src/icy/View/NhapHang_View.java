...
import icy.*;
import icy.Controller.ChiTietPhieuNhap_Controller;
import icy.Controller.DangKy_Controller;
import icy.Controller.NhaCungCap_Controller;
import icy.Controller.NhapHang_Controller;
import icy.Controller.PhieuNhap_Controller;
import icy.Controller.ThamSoChung_Controller;
import icy.Model.ChiTietPhieuNhap_DTO;
import icy.Model.DangKyDTO;
import icy.Model.NhaCungCap_DTO;
import icy.Model.NhapHang_DTO;
import icy.Model.PhieuNhap_DTO;
 ...
 * @author HP
 */
public class NhapHang_View extends javax.swing.JFrame {
    DefaultTableModel modelKho=new DefaultTableModel();
    public static NhapHang_View NH=new NhapHang_View();
    NhapHang_Controller bus=new NhapHang_Controller();
    NhapHang_DTO sptk=new NhapHang_DTO();
    ChiTietPhieuNhap_Controller busPN=new ChiTietPhieuNhap_Controller();
    ...
    PhieuNhap_Controller busPNH = new PhieuNhap_Controller();
    PhieuNhap_DTO pnh = new PhieuNhap_DTO();
    static NhaCungCap_View ncc_gui=new NhaCungCap_View();
    int d=0;//Biến dùng để khi nhập hàng sẽ tự động tăng lên (Mã PN) trong đó các spham nhập sẽ có cùng MãPN
    static int x=PhieuNhap_Controller.dspnh.size()+1;//Biến dùng gán Mã PN trong ds sản phẩm khi nhập
    ...
    public void DocTS(){
        ThamSoChung_Controller b = new ThamSoChung_Controller();
        ThamSoChung_DTO ts = new ThamSoChung_DTO();
        if(ThamSoChung_Controller.dsts==null){
            b.DocTS();
        }
        ts=b.dsts.get(0);
    ...
    /*======================================KHO Hàng==================================*/
    public void DocKho(){
        if (NhapHang_Controller.dstk == null) {
            bus.DocSPTonKho();
        }
        //cập nhật Jtable
        ...
        for (NhapHang_DTO sptk : NhapHang_Controller.dstk) {
    ...
    private void tbnXoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_tbnXoaActionPerformed
        ...
        if (i == -1) {
            ...
            if (x == JOptionPane.YES_OPTION) {
                if (i >= 0) {
                    ChiTietPhieuNhap_Controller.dsHoaDonNhap.remove(i);
                    modelPN.removeRow(i);
                    tblPhieuNhap.setModel(modelPN);
                    txtTongPhiNhap.setText(String.valueOf(tam));
                }
    ...
    public void DocCTPhieuNhap(){
        if (ChiTietPhieuNhap_Controller.dspn == null) {
            busPN.DocCTPN();
        }
        //cập nhật Jtable
        Vector header = new Vector();
        ...
        for (ChiTietPhieuNhap_DTO pn : ChiTietPhieuNhap_Controller.dspn) {
            //nạp dữ liệu cho mỗi row
            Vector row = new Vector();
            row.add(pn.MaPN);
            row.add(pn.MaSP);
...
/*======================================Phiếu Nhập==================================*/
    ...
    public void DocPhieuNhapHang(){
        if (PhieuNhap_Controller.dspnh == null) {
            busPNH.DocPhieuNhap();
        }
        //cập nhật Jtable
        Vector header = new Vector();
        ...
        for (PhieuNhap_DTO pnh : PhieuNhap_Controller.dspnh) {
            //nạp dữ liệu cho mỗi row
            Vector row = new Vector();
            row.add(pnh.MaPN);
            row.add(pnh.NgayLap);
    ...
    private void btnNhapActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnNhapActionPerformed
        if (ChiTietPhieuNhap_Controller.dsHoaDonNhap.size()>0 && KiemTraGiaTriNCC()==true) {
            d=PhieuNhap_Controller.dspnh.size()+1;
            ...
            for (int k = 0; k < ChiTietPhieuNhap_Controller.dsHoaDonNhap.size(); k++) {
                pn.MaSP = mPN.getValueAt(k, 1).toString();
                for (int j = 0; j < NhapHang_Controller.dstk.size(); j++) {
            ...
            ChiTietPhieuNhap_Controller.dsHoaDonNhap.removeAll(ChiTietPhieuNhap_Controller.dsHoaDonNhap);
            ((DefaultTableModel)tblPhieuNhap.getModel()).setRowCount(0);
            txtTongPhiNhap.setText("");
            JOptionPane.showMessageDialog(this, "Nhập hàng thành công!");
        } else {
            if (ChiTietPhieuNhap_Controller.dsHoaDonNhap.size() == 0) {
...
//-------------Khi enter chuyển qua text kế tiếp-----------//
    ...
    private void btnDangXuatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDangXuatActionPerformed
        ...
        int output = JOptionPane.showConfirmDialog(NH, 
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(NhapHang_View.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
