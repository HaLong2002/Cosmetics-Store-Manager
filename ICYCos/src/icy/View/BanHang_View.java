...
public class BanHang_View extends javax.swing.JFrame {
    ...
    BanHang_Controller bhbus = new BanHang_Controller();
    ChiTietHoaDon_DTO cthd = new ChiTietHoaDon_DTO();
    ChiTietHoaDon_Controller cthdbus = new ChiTietHoaDon_Controller();
    HoaDon_DTO hd = new HoaDon_DTO();
    HoaDon_Controller hdbus = new HoaDon_Controller();
    CTKhuyenMai_Controller kmbus =new CTKhuyenMai_Controller();
    QLNV_Controller nvbus = new QLNV_Controller();
    int d = 0;
    static int x = HoaDon_Controller.dshoadon.size() + 1;//Biến dùng gán Mã HD trong ds sản phẩm khi mua
    ...
    public void docdssp() {
        if (BanHang_Controller.dssp == null) {
            bhbus.DocSPTonKho();
        }
        ...
        for (SanPham_DTO sp : BanHang_Controller.dssp) {
    ...
    public void DocCTHD() {
        if (ChiTietHoaDon_Controller.dshd == null) {
            cthdbus.DocCTHD();
        }
        //cập nhật Jtable
        ...
        for (ChiTietHoaDon_DTO cthd : ChiTietHoaDon_Controller.dshd) {
    ...
    public void DocHoaDon() {
        if (HoaDon_Controller.dshoadon == null) {
            hdbus.DocHoaDon();
        }
        //cập nhật Jtable
        Vector header = new Vector();
        ...
        for (HoaDon_DTO hd : HoaDon_Controller.dshoadon) {
            //nạp dữ liệu cho mỗi row
            Vector row = new Vector();
            row.add(hd.MaHD);
            row.add(hd.NgayLap);
    ...
    private void btnDangXuatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDangXuatActionPerformed
        ...
        int output = JOptionPane.showConfirmDialog(BH, 
    ...
    private void btnXoaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXoaActionPerformed
        ...
        if (i == -1) {
            ...
            if (x == JOptionPane.YES_OPTION) {
                if (i >= 0) {
                    ChiTietHoaDon_Controller.dsHoaDon.remove(i);
                    modelGH.removeRow(i);
                    tblGioHang.setModel(modelGH);
                    txtTongPhiNhap.setText(String.valueOf(tam));
                }
    ...
    private void btnXuatActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnXuatActionPerformed
        if (ChiTietHoaDon_Controller.dsHoaDon.size() > 0 == true) {
            d = HoaDon_Controller.dshoadon.size() + 1;
            ...
            for (int k = 0; k < ChiTietHoaDon_Controller.dsHoaDon.size(); k++) {
                cthd.MaSP = mGH.getValueAt(k, 1).toString();
                for (int j = 0; j < BanHang_Controller.dssp.size(); j++) {
            ...
            ChiTietHoaDon_Controller.dsHoaDon.removeAll(ChiTietHoaDon_Controller.dsHoaDon);
            ((DefaultTableModel) tblGioHang.getModel()).setRowCount(0);
            txtTongPhiNhap.setText("");
            JOptionPane.showMessageDialog(this, "Nhập hàng thành công!");
        } else {
            if (ChiTietHoaDon_Controller.dsHoaDon.size() == 0) {
    ...
    private void tblCTHDMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tblCTHDMouseClicked
        ...
        if (i >= 0) {
            cthd = ChiTietHoaDon_Controller.dshd.get(i);
